...
**Foundation Authority**: The world foundation (`.worldbuild/memory/foundation.md`) is **non-negotiable** within this analysis scope. Foundation conflicts are automatically CRITICAL and require adjustment of the world, outline, or chapters—not reinterpretation or ignoring of established rules.

## Execution Steps

### 1. Initialize Analysis Context

Run `.worldbuild/scripts/bash/check-prerequisites.sh --json --require-tasks --include-tasks` once from repo root and parse JSON for STORY_DIR and AVAILABLE_DOCS. Derive absolute paths:

- WORLD = STORY_DIR/world.md
- OUTLINE = STORY_DIR/outline.md
- CHAPTERS = STORY_DIR/chapters.md
- FOUNDATION = .worldbuild/memory/foundation.md (if exists)
